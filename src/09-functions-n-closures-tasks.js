...
function getPolynom(...arg) {
  return (x) => arg.reduce((acc, cur, i) => acc + (cur * (x ** (arg.length - 1 - i))), 0);
}
...
function memoize(func) {
  ...
  return (...arg) => {
    const keyCacheItem = JSON.stringify(arg);

    if (keyCacheItem in cache) {
      return cache[keyCacheItem];
    }
    const result = func.call(this, ...arg);
    cache[keyCacheItem] = result;
    return result;
  };
}
...
function logger(func, logFunc) {
  return (...arg) => {
    const argLog = JSON.stringify(arg).slice(1, -1);
