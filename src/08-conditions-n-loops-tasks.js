...
function isInsideCircle(circle, point) {
  const x2 = (circle.center.x - point.x) ** 2;
  const y2 = (circle.center.y - point.y) ** 2;
  return circle.radius ** 2 > x2 + y2;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${isStartIncluded ? '[' : '('}${aNew}, ${bNew}${isEndIncluded ? ']' : ')'}`;
}
...
function getDigitalRoot(num) {
  let peremen = num;
  while (peremen > 9) {
    peremen = `${peremen}`.split('').reduce((acc, cur) => acc + +cur, 0);
  }
  return peremen;
}
